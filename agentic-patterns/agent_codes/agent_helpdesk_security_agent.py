...
from utils.help_desk_pydatic_models import validate_helpdesk_llm_output, SecurityOutput
...
class SecuritySignature(dspy.Signature):
    """
    You are HELP-DESK security-agent. Input: a formatted string containing "---ORIGINAL_USER_REQUEST---" and "---OUTPUT---" (the router's JSON output). You must return ONLY valid JSON that conforms to the Pydantic model SecurityOutput with this schema: {"domain":"security","summary":string,"risk_level":"low|medium|high","indicators":[string],"recommended_actions":[{"who":"user|system|agent","action":string,"estimated_time_minutes":int,"priority":"low|medium|high"}],"required_info":[string],"escalation": {"escalate_to":"l2_support|dev_ops|engineering","reason":string} | null,"confidence":float}. Behavior: If router output's selected_agents does NOT include "security-agent", RETURN A FULL VALID MINIMAL SecurityOutput object. Example minimal valid object to return when not selected: {"domain":"security","summary":"Not Applicable","risk_level":"low","indicators":[],"recommended_actions":[],"required_info":[],"escalation":null,"confidence":0.0}. Focus on security incidents, data breaches, and risk assessments. Return only JSON matching SecurityOutput exactly. Ensure JSON parses.
    """
    text = dspy.InputField(desc="The formatted input containing original_user_request and router_output")
    security_output = dspy.OutputField(desc="JSON matching SecurityOutput schema")

class SecurityModule(dspy.Module):
    def __init__(self, system_prompt):
        super().__init__()
        self.security_expert = dspy.ChainOfThought(SecuritySignature.with_instructions(system_prompt))

    def forward(self, text):
        return self.security_expert(text=text)
...
class SampleAgent:
    def __init__(self, subject, context: Context) -> None:
        ...
        self.module = SecurityModule(self.persona_default_system_message)
    ...
    def _execute_worker(self, input_text, original_user_request, router_output, model_name, session_id):
        ...
        if "security-agent" not in selected_agents:
            log.info("security-agent not selected by router. Returning minimal output.")
            return self.content_parser.get_minimal_output()
        
        log.info(f"[security-agent] Processing request: {original_user_request}")
        ...
        out_security = result.security_output
        if isinstance(out_security, str):
            try:
                out_security = json.loads(out_security)
            except:
                pass
        return out_security
    ...
    def on_data(self, task: AgentTask) -> AgentResult:
        try:
            ...
            # 1. Run worker
            out_security = self._execute_worker(input_text, original_user_request, router_output, model_name, session_id)
            log.info(f"Security Result: {out_security}")
            
            # 2. Prepare for next agent (synthesizer)
            try:
                next_agent_id = self.agents_config["help-desk"]["security-agent"]["next_agent"]
            except (KeyError, TypeError):
                log.warning("next_agent not found in config for security-agent. Defaulting to synthesizer-agent.")
                next_agent_id = "synthesizer-agent"

            log.info(f"Submitting result to {next_agent_id} via {communication_type}")

            job_data = {
                "security_output": out_security,
                "session_id": session_id,
                "model_name": model_name,
            ...
            log.exception(f"Error in Security Agent: {e}")
