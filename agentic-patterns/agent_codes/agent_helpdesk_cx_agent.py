...
from utils.help_desk_pydatic_models import validate_helpdesk_llm_output, CXOutput

log = logging.getLogger(__name__)

# --- DSPy Signatures ---
class CXSignature(dspy.Signature):
    """
    You are HELP-DESK cx-agent. Input: a formatted string containing "---ORIGINAL_USER_REQUEST---" and "---OUTPUT---" (the router's JSON output). You must return ONLY valid JSON that conforms to the Pydantic model CXOutput with this schema: {"domain":"cx","summary":string,"sentiment_score":number,"urgency_level":"low|medium|high|critical","actions":[{"who":"user|system|agent","action":string,"estimated_time_minutes":int,"priority":"low|medium|high"}],"retention_risk":boolean,"escalation": {"escalate_to":string,"reason":string} | null}. Behavior: If router output's selected_agents does NOT include "cx-agent", RETURN A FULL VALID MINIMAL CXOutput object. Example minimal valid object to return when not selected: {"domain":"cx","summary":"Not Applicable","sentiment_score":0.5,"urgency_level":"low","actions":[],"retention_risk":false,"escalation":null}. Assess the user's emotional state and provide empathetic but concise responses. Return only JSON matching CXOutput exactly. Ensure JSON parses.
    """
    text = dspy.InputField(desc="The formatted input containing original_user_request and router_output")
    cx_output = dspy.OutputField(desc="JSON matching CXOutput schema")

class CXModule(dspy.Module):
    def __init__(self, system_prompt):
        super().__init__()
        self.cx_expert = dspy.ChainOfThought(CXSignature.with_instructions(system_prompt))

    def forward(self, text):
        return self.cx_expert(text=text)
...
class SampleAgent:
    def __init__(self, subject, context: Context) -> None:
        ...
        self.module = CXModule(self.persona_default_system_message)
    ...
    def _execute_worker(self, input_text, original_user_request, router_output, model_name, session_id):
        ...
        if "cx-agent" not in selected_agents:
            log.info("cx-agent not selected by router. Returning minimal output.")
            return self.content_parser.get_minimal_output()
        
        log.info(f"[cx-agent] Processing request: {original_user_request}")
        ...
        out_cx = result.cx_output
        if isinstance(out_cx, str):
            try:
                out_cx = json.loads(out_cx)
            except:
                pass
        return out_cx
    ...
    def on_data(self, task: AgentTask) -> AgentResult:
        try:
            ...
            # 1. Run worker
            out_cx = self._execute_worker(input_text, original_user_request, router_output, model_name, session_id)
            log.info(f"CX Result: {out_cx}")
            
            # 2. Prepare for next agent (synthesizer)
            try:
                next_agent_id = self.agents_config["help-desk"]["cx-agent"]["next_agent"]
            except (KeyError, TypeError):
                log.warning("next_agent not found in config for cx-agent. Defaulting to synthesizer-agent.")
            ...
            log.exception(f"Error in CX Agent: {e}")
