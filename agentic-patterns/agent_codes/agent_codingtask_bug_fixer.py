...
class BugFixerSignature(dspy.Signature):
    """
    You are Agent3 ‘BugRefiner’. Given normalized_text that may include a short code snippet and notes,
    produce a complete, actionable bug report without inventing facts.
    Tasks:
    1) Extract and structure facts into sections: Title, Summary, Steps to Reproduce, Expected, Actual,
       Environment, Impact, Attachments/Logs, Workarounds; write ‘Unknown’ if a field is missing.
    2) Keep between 120 and 180 words excluding code.
    3) Preserve versions, paths, error strings, and flags exactly; do not change semantics.
    4) If the code suggests likely root causes, add a ‘Suspected Causes’ section with clearly marked hypotheses.
    
    The improved_markdown must use H2 headings (## Title, ## Summary, ## Steps to Reproduce, ## Expected,
    ## Actual, ## Environment, ## Impact, ## Attachments/Logs, ## Workarounds, ## Suspected Causes).

    Output fields:
    1) improved_markdown: Report in Markdown with H2 headings
    2) corrected_code: The full corrected code snippet. You MUST provide the complete fixed code here.
    3) key_sections_present: List of present sections (comma-separated)
    4) actionable_checks: List of status flags (comma-separated), e.g., ‘has_steps’, ‘has_environment’, ‘has_expected_vs_actual’, ‘has_logs_or_code’.
    """
    ...
    corrected_code = dspy.OutputField(desc="The complete corrected code snippet")
...
# --- DSPy Modules ---
class BugFixerModule(dspy.Module):
    def __init__(self, system_prompt):
        super().__init__()
        self.cot = dspy.ChainOfThought(BugFixerSignature.with_instructions(system_prompt))
...
class SampleAgent:
    ...
    def __init__(self, subject, context: Context) -> None:
        ...
        self.module = BugFixerModule(self.persona_default_system_message)
    ...
    def on_data(self, task: AgentTask) -> AgentResult:
        try:
            ...
            log.info(f"Processing bug report for normalized text: {normalized_text[:100]}...")
            ...
            corrected_code = getattr(result, "corrected_code", "None")
            ...
            state["bug_report_json"] = output_json_str
            ...
            state["bug_report"] = output_json_str
            ...
            log.exception(f"Error in Bug Fixer Agent: {e}")
