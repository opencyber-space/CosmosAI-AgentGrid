...
from utils.help_desk_pydatic_models import validate_helpdesk_llm_output, BillingOutput
...
class BillingSignature(dspy.Signature):
    """
    You are HELP-DESK billing-agent. Input: a formatted string containing "---ORIGINAL_USER_REQUEST---" and "---OUTPUT---" (the router's JSON output). You must return ONLY valid JSON that conforms to the Pydantic model BillingOutput with this schema: {"domain":"billing","summary":string,"total_amount_under_dispute":number | null,"currency":string,"actions":[{"who":"user|system|agent","action":string,"estimated_time_minutes":int,"priority":"low|medium|high"}],"refund_eligibility":"eligible|ineligible|pending_review","escalation": {"escalate_to":string,"reason":string} | null}. Behavior: If router output's selected_agents does NOT include "billing-agent", RETURN A FULL VALID MINIMAL BillingOutput object. Example minimal valid object to return when not selected: {"domain":"billing","summary":"Not Applicable","total_amount_under_dispute":null,"currency":"USD","actions":[],"refund_eligibility":"ineligible","escalation":null}. Analyze specific charges, invoice dates, and refund policies if mentioned. Return only JSON matching BillingOutput exactly. Ensure JSON parses.
    """
    text = dspy.InputField(desc="The formatted input containing original_user_request and router_output")
    billing_output = dspy.OutputField(desc="JSON matching BillingOutput schema")

class BillingModule(dspy.Module):
    def __init__(self, system_prompt):
        super().__init__()
        self.billing_expert = dspy.ChainOfThought(BillingSignature.with_instructions(system_prompt))

    def forward(self, text):
        return self.billing_expert(text=text)
...
class ContentParser:
    ...
    def get_minimal_output():
        return {
            ...
            "total_amount_under_dispute": None,
            "currency": "USD",
            "actions": [],
...
class SampleAgent:
    def __init__(self, subject, context: Context) -> None:
        ...
        self.module = BillingModule(self.persona_default_system_message)
    ...
    def _execute_worker(self, input_text, original_user_request, router_output, model_name, session_id):
        ...
        if "billing-agent" not in selected_agents:
            log.info("billing-agent not selected by router. Returning minimal output.")
            return self.content_parser.get_minimal_output()
        
        log.info(f"[billing-agent] Processing request: {original_user_request}")
        ...
        out_billing = result.billing_output
        if isinstance(out_billing, str):
            try:
                out_billing = json.loads(out_billing)
            except:
                pass
        return out_billing
    ...
    def on_data(self, task: AgentTask) -> AgentResult:
        try:
            ...
            # 1. Run worker
            out_billing = self._execute_worker(input_text, original_user_request, router_output, model_name, session_id)
            log.info(f"Billing Result: {out_billing}")
            
            # 2. Prepare for next agent (synthesizer)
            try:
                next_agent_id = self.agents_config["help-desk"]["billing-agent"]["next_agent"]
            except (KeyError, TypeError):
                log.warning("next_agent not found in config for billing-agent. Defaulting to synthesizer-agent.")
                next_agent_id = "synthesizer-agent"

            log.info(f"Submitting result to {next_agent_id} via {communication_type}")

            job_data = {
                "billing_output": out_billing,
                "session_id": session_id,
                "model_name": model_name,
            ...
            log.exception(f"Error in Billing Agent: {e}")
