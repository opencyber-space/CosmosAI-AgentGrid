...
class FinancialProfileSignature(dspy.Signature):
    """
    You are a senior credit analyst at a commercial bank.

    Analyse the loan application text and extract a structured financial profile
    of the applicant. Pay close attention to any inconsistencies or signals that
    might indicate misrepresentation or fraud.
    ...
    financial_profile = dspy.OutputField(desc="Structured JSON with the extracted financial profile")

class FinancialProfileModule(dspy.Module):
    def __init__(self, system_prompt):
        super().__init__()
        self.extractor = dspy.Predict(FinancialProfileSignature)

    def forward(self, application_text):
        return self.extractor(application_text=application_text)
...
class FinancialProfileAgent:
    def __init__(self, subject, context: Context) -> None:
        ...
        self.module = FinancialProfileModule(self.persona_default_system_message)
    ...
    def _execute_worker(self, application_text, model_name, session_id):
        with self.model_context.get_context(model_name=model_name, session_id=session_id):
            return self.module.forward(application_text=application_text)
    ...
    def on_preprocess(self, task: AgentTask) -> Optional[List[AgentTask]]:
        if not task.job_data.get("text"):
            log.warning("Task %s — missing 'text' in job_data, skipping.", task.task_id)
            return None
        return [task]
    ...
    def on_data(self, task: AgentTask) -> AgentResult:
        try:
            ...
            result = self._execute_worker(raw_text, model_name, llm_session_id)
            ...
            return AgentResult(
                ...
                job_output=parsed,
                job_output_metadata={
                    "creditworthiness": parsed.get("creditworthiness"),
