...
TEAM_MAPPING = {
    "Risk Assessment Multi-Level Workflow": [
...
#     "Risk Assessment Multi-Level Workflow": [
#         "my-router-agent",
#         "my-financial-profile-agent",
#         "my-market-risk-agent",
#         "my-collateral-evaluator-agent",
#         "my-fraud-investigation-workflow",
#         "my-loan-decision-agent"
#     ],
#     "Fraud Investigation Sub-workflow": [
#         "my-transaction-history-agent",
#         "my-identity-verification-agent",
#         "my-fraud-score-agent"
#     ]
# }
...
def draw_response_card(op):
    ...
    ts = datetime.datetime.fromtimestamp(precise_t).strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
...
def display_team_timeline(team_name, agent_ids):
    ...
    # Sort ALL messages globally by precise time (descending: newest at the top, or ascending if we want reading order)
    ...
    for op in all_team_messages:
        ...
        with cols[col_idx]:
            ...
            if isinstance(raw_json, str):
                ...
                collapsible_html = f'<pre style="margin: 0; padding: 0;"><code>{html.escape(raw_json)}</code></pre>'
            else:
            ...
            html_string = f"""<div class="condensed-item"><div style="font-weight: 600; margin-bottom: 4px; display: flex; justify-content: space-between;"><span>{header_label}</span><span style="color: #888; font-weight: 400; font-size: 12px;">[{ts}]</span></div><div style="font-size: 12px; color: #0066cc; margin-bottom: 2px;">{direction_label}</div><div style="font-size: 11px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color: #888;">{preview_text}...</div><div class="tooltip-text" style="max-height: 500px; overflow-y: auto; pointer-events: auto;"><div style="margin-bottom: 10px; padding-bottom: 10px; border-bottom: 1px solid #444;"><span style="color: #9cdcfe; font-weight: bold;">{header_label}</span> <br/><span style="color: #9cdcfe; font-weight: bold;">Time:</span> {ts}</div><span style="color: #9cdcfe; font-weight: bold;">{direction_label}:</span><br/><div style="background-color: #1e1e1e; padding: 10px; border-radius: 4px; font-family: monospace; font-size: 13px;">{collapsible_html}</div></div></div>"""
            st.markdown(html_string, unsafe_allow_html=True)
