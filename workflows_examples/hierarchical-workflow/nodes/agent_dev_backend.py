...
class CodePlanSignature(dspy.Signature):
    ...
    ### TASK
    Generate a complete project code plan consisting of a list of files necessary to build the backend requirements. Include dependency files like `package.json` or `requirements.txt`, but DO NOT include any bash or setup scripts (e.g., `setup.sh`).
    Keep the plan concise. Do NOT generate huge nested arrays.

    ### OUTPUT
    Output EXACTLY a valid JSON block mirroring this syntax:
    {"code_plan": [{"filename": "app.js", "description": "foo"}, {"filename": "setup.sh", "description": "bar"}]}
    ...
    dev_pointers = dspy.InputField(desc="Specific backend architecture/API implementation pointers from the Team Lead")
...
class CodeFileSynthesisSignature(dspy.Signature):
    """
    ### ROLE
    You are the Backend Developer writing production-ready code.
    ...
    CRITICAL: Avoid repeating patterns, deep directory trees, or recursive logic. Be concise and production-ready.
    ...
    """
    architecture = dspy.InputField(desc="The system architecture design")
    dev_pointers = dspy.InputField(desc="Specific backend pointers from the Team Lead")
...
class BackendCodePlanModule(dspy.Module):
...
class BackendCodeSynthesisModule(dspy.Module):
    def __init__(self, system_prompt):
        super().__init__()
...
class BackendDevAgent:
    def __init__(self, subject, context: Context) -> None:
        ...
        self.plan_module = BackendCodePlanModule(self.persona_default_system_message)
        self.synthesis_module = BackendCodeSynthesisModule(self.persona_default_system_message)
    ...
    def on_preprocess(self, task: AgentTask) -> Optional[List[AgentTask]]:
        ...
        if task_type != "execute_task":
            log.warning("Task %s has task_type %s, which is not execute_task, skipping Backend Developer.", task.task_id, task_type)
            return None
        return [task]
    ...
    def on_data(self, task: AgentTask) -> AgentResult:
        try:
            ...
            dev_pointers = data.get("backend_pointers") or data.get("dev_pointers") or "Build standard secure backend APIs."
            ...
            with self.model_context.get_context(model_name=model_name, session_id=llm_session_id):
                ...
                log.info("Backend Dev generating code plan...")
                ...
                try:
                    ...
                    log.error(f"Backend Dev failed to parse code_plan JSON: {e}")
                    code_plan = []
                
                # Step 2: Iteratively Synthesize Code Files
                log.info(f"Backend Dev plan established: {len(code_plan)} files. Synthesizing...")
                synthesized_files = {}
                file_urls = {}
                local_dir = f"/tmp/dev_output/backend/{task_id}/"
            ...
            output_data = {
                ...
                "backend_summary": f"Successfully planned and generated {len(synthesized_files)} files.",
                "code_plan": code_plan,
                "file_urls": file_urls
            }
