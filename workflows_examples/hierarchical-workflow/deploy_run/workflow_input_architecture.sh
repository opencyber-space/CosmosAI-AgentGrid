...
RESPONSE=$(curl -s "${API_BASE_URL}/api/runtime-workflows?workflow_uri=workflow-architecture-team:1.0.0-production&limit=20&skip=0")
...
SESSION_ID="session-architecture-demo-debate-"$(( RANDOM % 100 ))
TASK_ID="task-architecture-demo-debate-"$(( RANDOM % 100 ))
USER_REQUEST="Design the smart coffee machine architecture blueprint"
...
SESSION_ID="session-architecture-demo-debate-"$(( RANDOM % 100 ))
# --- Request 2: execute_task (Start Debate Loop) ---
...
echo "---------------------------------------------------"
echo "Sending Request 2 (execute_task - Trigger Debate Loop):"
