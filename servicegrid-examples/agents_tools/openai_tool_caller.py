...
# Fields pulled from the tool schema for OpenAI function definitions.
# Only these are sent to OpenAI — not the full schema.
_SEARCH_FIELDS = ("tool_id", "tool_search_description", "tool_tags", "tool_api_spec")


def _sanitize_fn_name(tool_id: str) -> str:
    """Map a tool_id to a valid OpenAI function name ([a-zA-Z0-9_-]{1,64})."""
    return re.sub(r"[^a-zA-Z0-9_-]", "_", tool_id)[:64]


def _build_openai_function(tool: Dict) -> Dict:
    """
    Build one OpenAI tool-calling function definition from selective schema fields.

    Uses:
      tool_id                            → function name (sanitized)
      tool_search_description + tool_tags → description sent to OpenAI
...
class OpenAIToolCaller:
    """
    Integrates OpenAI function-calling with the tools registry so that a natural-
    ...
    def _call_openai(
        ...
        """
        Send prompt (+ optional input_dict) to OpenAI with all tool definitions
        and return (tool_id, formatted_args).

        OpenAI selects the matching function and fills the arguments according to
        ...
        raw_args: str = tool_call.function.arguments  # JSON string from OpenAI

        tool_id = name_map.get(fn_name)
        if not tool_id:
            raise ValueError(f"OpenAI returned unknown function name: '{fn_name}'")

        formatted_args: Dict = json.loads(raw_args)
        logger.info(f"OpenAI selected tool='{tool_id}', args={formatted_args}")
    ...
    def resolve(
        ...
        """
        Use OpenAI to select the right tool and format *input_dict* against its
        ...
        *tools* — pre-loaded tool records to use instead of fetching from the
        ...
        return self._call_openai(prompt, tools, input_dict)
    ...
    def search_and_execute_tool(
        ...
        via OpenAI function-calling, execute the tool, and return the output.

        Workflow
        --------
        1. Fetch all tools from the registry.
        2. Ask OpenAI (with all tools as function definitions) to select the right
           tool and structure *input_dict* according to its execute.input schema.
        3. Parse the JSON-encoded arguments returned by OpenAI back into a dict.
        ...
        input_dict : dict
            Raw input data; OpenAI will validate and reshape it to match the
        ...
        tool_id, formatted_args = self._call_openai(prompt, tools, input_dict)
